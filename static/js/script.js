let counter = 1;
// Distinct colors matching the vibrant palette in 3_2.png
...
function addProcess() {
    ...
    div.className = "grid grid-cols-[100px_1fr_1fr_60px] gap-4 items-center animate-fade-in";
    div.innerHTML = `
        <div class="h-16 rounded-xl flex items-center justify-center text-white font-extrabold text-3xl shadow-sm" style="background-color: ${color}">${name}</div>
        <input type="number" placeholder="Arrival" class="at-input input-field text-xl" min="0">
        <input type="number" placeholder="Burst" class="bt-input input-field text-xl" min="1">
        <button onclick="removeThis(this)" aria-label="Delete Process" class="text-red-500 hover:scale-110 transition-transform flex justify-center">
...
function renderScale(gantt, processes) {
    ...
    gantt.forEach(step => {
        ...
        // Adjust height based on total steps to fit container, or fixed height
