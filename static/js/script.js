...
function addProcess() {
    ...
    div.className = "grid grid-cols-[80px_1fr_1fr_60px] gap-4 items-center p-4 hover:bg-white/[0.02] transition-colors";
    div.innerHTML = `
        <div class="text-xs font-mono font-bold" style="color:${color}">${name}</div>
        <input type="number" class="at-input input-field px-4 py-2 rounded text-sm" placeholder="AT">
        <input type="number" class="bt-input input-field px-4 py-2 rounded text-sm" placeholder="BT">
        <button onclick="this.closest('div').remove(); updateUI();" class="text-slate-600 hover:text-red-400 flex justify-end transition-colors">
...
function renderScale(gantt, processes) {
    ...
    [...gantt].reverse().forEach((step, index) => {
        const p = processes.find(proc => proc.name === step.id);
        if(!p) return;
        const block = document.createElement('div');
        block.className = "w-full border-t border-slate-950 flex items-center px-4 relative animate-fill shrink-0";
        block.style.backgroundColor = p.color;
        block.style.height = "32px";
        block.style.animationDelay = `${index * 0.02}s`;
        block.innerHTML = `<span class="text-[10px] font-mono font-bold text-white uppercase">${step.id}</span>`;
        scale.appendChild(block);
    });
}

async function handleExecution() {
    const isValid = await updateUI();
    // Use globalThis for environmental compatibility
    if (isValid) {
        globalThis.location.href = '/statistics';
    } else {
        alert("Input Arrival and Burst times first.");
    }
}
